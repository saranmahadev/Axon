...
class RedisAdapter(StorageAdapter):
    ...
    def _entry_to_redis(self, entry: MemoryEntry) -> dict[str, str]:
        ...
        data = {
            ...
            "created_at": entry.metadata.created_at.isoformat() if entry.metadata.created_at else "",
            "user_id": entry.metadata.user_id or "",
            "session_id": entry.metadata.session_id or "",
            "source": entry.metadata.source,
            "privacy_level": entry.metadata.privacy_level,
            "importance": str(entry.metadata.importance) if entry.metadata.importance is not None else "",
    ...
    def _matches_filter(self, entry: MemoryEntry, filter: Filter) -> bool:
        ...
        if filter.min_importance is not None:
            if entry.metadata.importance is None or entry.metadata.importance < filter.min_importance:
                return False
        if filter.max_importance is not None:
            if entry.metadata.importance is None or entry.metadata.importance > filter.max_importance:
                return False
