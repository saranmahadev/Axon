...
class PineconeAdapter(StorageAdapter):
    ...
    async def _ensure_index(self, dimension: int) -> None:
        ...
        if self._dimension is not None and self._dimension != dimension:
            raise ValueError(
                f"Dimension mismatch: index has {self._dimension}, got {dimension}"
            )
    ...
    def _filter_to_pinecone(self, filter_obj: Filter) -> Optional[dict]:
        ...
        if filter_obj.date_range:
            if filter_obj.date_range.start:
                conditions.append({
                    "created_at": {"$gte": filter_obj.date_range.start.timestamp()}
                })
            if filter_obj.date_range.end:
                conditions.append({
                    "created_at": {"$lte": filter_obj.date_range.end.timestamp()}
                })
    ...
    async def query(
        ...
        matches = results.get("matches", []) if isinstance(results, dict) else (results.matches if hasattr(results, 'matches') else [])
