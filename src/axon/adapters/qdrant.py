...
from ..models import DateRange, Filter, MemoryEntry
...
class QdrantAdapter(StorageAdapter):
    ...
    def _entry_to_point(self, entry: MemoryEntry) -> PointStruct:
        ...
        payload = {
            ...
            "provenance": json.dumps([p.model_dump(mode='json') for p in entry.metadata.provenance])
        }
    ...
    def _filter_to_qdrant(self, filter_obj: Filter) -> Optional[models.Filter]:
        ...
        if filter_obj.user_id:
            conditions.append(
                models.FieldCondition(
                    key="user_id",
                    match=models.MatchValue(value=filter_obj.user_id)
                )
            )
        ...
        if filter_obj.session_id:
            conditions.append(
                models.FieldCondition(
                    key="session_id",
                    match=models.MatchValue(value=filter_obj.session_id)
                )
            )
        ...
        if filter_obj.privacy_level:
            conditions.append(
                models.FieldCondition(
                    key="privacy_level",
                    match=models.MatchValue(value=filter_obj.privacy_level)
                )
            )
        ...
        if filter_obj.tags:
            conditions.append(
                models.FieldCondition(
                    key="tags",
                    match=models.MatchAny(any=filter_obj.tags)
                )
            )
        ...
        if filter_obj.min_importance is not None:
            conditions.append(
                models.FieldCondition(
                    key="importance",
                    range=models.Range(gte=filter_obj.min_importance)
                )
            )
        if filter_obj.max_importance is not None:
            ...
            if filter_obj.min_importance is not None:
                conditions[-1] = models.FieldCondition(
                    key="importance",
                    range=models.Range(
                        gte=filter_obj.min_importance,
                        lte=filter_obj.max_importance
                    )
                )
            else:
                conditions.append(
                    models.FieldCondition(
                        key="importance",
                        range=models.Range(lte=filter_obj.max_importance)
                    )
                )
    ...
    def count(self) -> int:
        ...
        try:
            ...
            raise RuntimeError("count() cannot be called from async context, use await count_async()")
        except RuntimeError:
    ...
    def query_sync(
        ...
        try:
            loop = asyncio.get_running_loop()
            raise RuntimeError("query_sync() cannot be called from async context, use await query()")
        except RuntimeError:
    ...
    def delete_sync(self, entry_id: str) -> bool:
        ...
        try:
            loop = asyncio.get_running_loop()
            raise RuntimeError("delete_sync() cannot be called from async context, use await delete()")
        except RuntimeError:
    ...
    def bulk_save_sync(self, entries: list[MemoryEntry]) -> None:
        ...
        try:
            loop = asyncio.get_running_loop()
            raise RuntimeError("bulk_save_sync() cannot be called from async context, use await bulk_save()")
        except RuntimeError:
    ...
    def reindex_sync(self) -> None:
        ...
        try:
            loop = asyncio.get_running_loop()
            raise RuntimeError("reindex_sync() cannot be called from async context, use await reindex()")
        except RuntimeError:
