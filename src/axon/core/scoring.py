...
class ScoringConfig:
    ...
    def __post_init__(self):
        ...
        if not 0.0 <= self.promotion_threshold <= 1.0:
            raise ValueError(f"promotion_threshold must be between 0.0 and 1.0")
        if not 0.0 <= self.demotion_threshold <= 1.0:
            raise ValueError(f"demotion_threshold must be between 0.0 and 1.0")
        ...
        if abs(promo_sum - 1.0) > 0.1:
            import warnings
            warnings.warn(f"Promotion weights sum to {promo_sum:.2f}, recommended ~1.0")
        ...
        if abs(demo_sum - 1.0) > 0.1:
            import warnings
            warnings.warn(f"Demotion weights sum to {demo_sum:.2f}, recommended ~1.0")
...
class ScoringEngine:
    ...
    def _calculate_velocity_score(self, entry: MemoryEntry) -> float:
        ...
        now = datetime.now(created_at.tzinfo) if (hasattr(created_at, 'tzinfo') and created_at.tzinfo) else datetime.now()
    ...
    def explain_score(self, score_details: Dict[str, float]) -> str:
        ...
        for name, value in components.items():
            ...
            lines.append(
                f"  {name:20s}: {value:5.3f} × {weight:.2f} = {contribution:.3f}"
            )
