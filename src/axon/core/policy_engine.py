...
from axon.core.scoring import ScoringEngine, ScoringConfig
...
class PolicyEngine:
    ...
    def should_compact(self, tier_name: str) -> Tuple[bool, Dict[str, Any]]:
        """
        Determine if tier should undergo compaction.
        
        Checks:
        - Tier capacity vs max_entries
        ...
        Returns:
            ...
            details contains: entry_count, max_entries, threshold, reason
        
        Example:
            >>> should, details = engine.should_compact("session")
            >>> if should:
            ...     print(f"Compact session tier: {details['entry_count']} entries")
        """
        ...
        # If no max_entries or compaction_threshold, no compaction needed
        if policy.max_entries is None:
            return False, {
                "should_compact": False,
                "tier": tier_name,
                "reason": "Tier has no max_entries limit (unlimited capacity)",
                "max_entries": None
            }
        ...
        if policy.compaction_threshold is None:
            return False, {
                "should_compact": False,
                "tier": tier_name,
                "reason": "Tier has no compaction_threshold configured",
                "max_entries": policy.max_entries,
                "compaction_threshold": None
            }
        
        # Get current entry count from registry statistics
        # Note: This is a placeholder - actual implementation would query registry
        # For now, we'll return the decision logic structure
        entry_count = 0  # TODO: Get from registry.get_statistics(tier_name)
        
        should_compact = entry_count >= policy.compaction_threshold
        ...
        details = {
            ...
            "capacity_utilization": entry_count / policy.max_entries if policy.max_entries else 0.0
        }
        
        if should_compact:
            details["reason"] = (
                f"Tier has {entry_count} entries, exceeds compaction threshold "
                f"of {policy.compaction_threshold}"
            )
        else:
            details["reason"] = (
                f"Tier has {entry_count} entries, below compaction threshold "
                f"of {policy.compaction_threshold}"
            )
        
        logger.debug(
            f"Compaction check for '{tier_name}': count={entry_count}, "
            f"threshold={policy.compaction_threshold}, decision={should_compact}"
        )
        
        return should_compact, details
    ...
    def check_overflow(self, tier_name: str) -> Tuple[bool, Dict[str, Any]]:
        ...
        if is_overflow:
            ...
            details["reason"] = (
                f"Tier has {entry_count} entries, within max_entries "
                f"of {policy.max_entries}"
            )
