...
class PersistentPolicy(Policy):
    ...
    tier_name: Literal["persistent"] = Field(
        default="persistent",
        description="Tier name is always 'persistent'"
    )
    
    adapter_type: Literal["chroma", "qdrant", "pinecone", "memory"] = Field(
        "chroma",
        description="Vector database for persistent storage"
    )
    
    ttl_seconds: Optional[int] = Field(
        None,
        description="Usually None (no expiration) or very long (days/months)"
    )
    
    compaction_threshold: Optional[int] = Field(
        10000,
        description="Compact after N entries (default: 10,000)",
        gt=0
    )
    
    compaction_strategy: Literal["count", "semantic", "importance", "time"] = Field(
        "count",
        description="Strategy for compacting memories"
    )
    
    enable_vector_search: Literal[True] = Field(
        default=True,
        description="Always enabled for persistent tier"
    )
    
    archive_adapter: Optional[str] = Field(
        None,
        description="Adapter for archived memories (e.g., 's3', 'gcs')"
    )
    
    @field_validator("compaction_threshold")
    @classmethod
    def validate_compaction_threshold(cls, v: Optional[int]) -> Optional[int]:
        """
    ...
    def validate_persistent_adapter(cls, v: str) -> str:
        ...
        if v not in ["chroma", "qdrant", "pinecone", "memory"]:
            raise ValueError(
                f"Persistent tier requires vector-capable adapter, got '{v}'"
            )
        return v
    
    @field_validator("ttl_seconds")
    @classmethod
    def validate_persistent_ttl(cls, v: Optional[int]) -> Optional[int]:
        """
        ...
        if v is not None and v < 86400:  # 1 day
            raise ValueError(
                "Persistent tier TTL should be at least 1 day (86400s) "
