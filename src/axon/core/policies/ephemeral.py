...
class EphemeralPolicy(Policy):
    ...
    tier_name: Literal["ephemeral"] = Field(
        default="ephemeral",
        description="Tier name is always 'ephemeral'"
    )
    
    adapter_type: Literal["redis", "memory"] = Field(
        "redis",
        description="Only in-memory adapters allowed for ephemeral tier"
    )
    
    ttl_seconds: int = Field(
        60,
        description="TTL between 5 seconds and 1 hour (default: 60s)",
        ge=5,
        le=3600
    )
    ...
    eviction_strategy: Literal["ttl"] = Field(
        default="ttl",
        description="Always TTL-based for ephemeral tier"
    )
    
    enable_vector_search: Literal[False] = Field(
        default=False,
        description="Vector search disabled for ephemeral tier"
    )
    ...
    def validate_ephemeral_adapter(cls, v: str) -> str:
        ...
        if v not in ["redis", "memory"]:
            raise ValueError(
                f"Ephemeral tier requires in-memory adapter, got '{v}'"
            )
        return v
