...
class SessionPolicy(Policy):
    ...
    tier_name: Literal["session"] = Field(
        default="session",
        description="Tier name is always 'session'"
    )
    
    adapter_type: Literal["redis", "memory", "chroma", "qdrant", "pinecone"] = Field(
        "redis",
        description="Adapter for session storage (typically Redis)"
    )
    
    ttl_seconds: Optional[int] = Field(
        600,
        description="TTL in seconds (default: 10 minutes, min: 60s if set)",
        ge=60
    )
    
    max_entries: Optional[int] = Field(
        1000,
        description="Maximum entries per session (default: 1000)",
        gt=0
    )
    
    overflow_to_persistent: bool = Field(
        False,
        description="Promote to persistent tier when max_entries reached"
    )
    
    enable_vector_search: bool = Field(
        True,
        description="Enable vector search if adapter supports it"
    )
    
    @field_validator("ttl_seconds")
    @classmethod
    def validate_session_ttl(cls, v: Optional[int]) -> Optional[int]:
        """
        ...
        if v is not None and v < 60:
            raise ValueError(
                "Session TTL should be at least 60 seconds "
                "(use ephemeral tier for shorter TTLs)"
            )
        return v
    
    @field_validator("max_entries")
    @classmethod
    def validate_max_entries(cls, v: Optional[int]) -> Optional[int]:
        """
    ...
    @classmethod
    def validate_vector_search_with_adapter(
        cls, v: bool, info
    ) -> bool:
        """
