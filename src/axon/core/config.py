...
class MemoryConfig(BaseModel):
    ...
    ephemeral: Optional[EphemeralPolicy] = Field(
        None,
        description="Ephemeral tier policy (optional)"
    )
    
    session: Optional[SessionPolicy] = Field(
        None,
        description="Session tier policy (optional)"
    )
    
    persistent: PersistentPolicy = Field(
        ...,
        description="Persistent tier policy (required)"
    )
    
    default_tier: Literal["ephemeral", "session", "persistent"] = Field(
        "session",
        description="Default tier for new memories"
    )
    
    enable_promotion: bool = Field(
        False,
        description="Auto-promote important memories to higher tiers"
    )
    ...
    def validate_default_tier_exists(self):
        ...
        if self.default_tier == "ephemeral" and self.ephemeral is None:
            raise ValueError(
                "default_tier is 'ephemeral' but ephemeral policy is not configured"
            )
        if self.default_tier == "session" and self.session is None:
            raise ValueError(
                "default_tier is 'session' but session policy is not configured"
            )
        return self
    ...
    def validate_promotion_requires_tiers(self):
        ...
        if self.enable_promotion:
            ...
            if tier_count < 2:
                raise ValueError(
                    "enable_promotion requires at least 2 tiers configured"
                )
        ...
        if self.enable_demotion:
            ...
            if tier_count < 2:
                raise ValueError(
                    "enable_demotion requires at least 2 tiers configured"
                )
    ...
    def get_policy(self, tier_name: str) -> Optional[EphemeralPolicy | SessionPolicy | PersistentPolicy]:
        """
