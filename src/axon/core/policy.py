...
class Policy(BaseModel):
    ...
    max_entries: Optional[int] = Field(
        None,
        description="Maximum entries before eviction (None = unlimited)",
        gt=0
    )
    
    compaction_threshold: Optional[int] = Field(
        None,
        description="Entry count that triggers compaction (None = disabled)",
        gt=0
    )
    
    eviction_strategy: Literal["ttl", "lru", "fifo", "importance"] = Field(
        "ttl",
        description="Strategy for removing entries when at capacity"
    )
    
    enable_vector_search: bool = Field(
        True,
        description="Whether this tier supports vector similarity search"
    )
    ...
    @classmethod
    def validate_compaction_reasonable(cls, v: Optional[int]) -> Optional[int]:
