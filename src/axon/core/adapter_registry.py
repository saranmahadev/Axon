...
class AdapterRegistry:
    ...
    async def _initialize_adapter(self, tier: str) -> None:
        ...
        if adapter is None:
            ...
            try:
                ...
                raise RuntimeError(
                    f"Failed to create adapter for tier '{tier}' "
                    f"(type: {adapter_type}): {e}"
                ) from e
        ...
        if hasattr(adapter, 'initialize') and callable(adapter.initialize):
            try:
                ...
                raise RuntimeError(
                    f"Failed to initialize adapter for tier '{tier}': {e}"
                ) from e
        else:
    ...
    async def close_all(self) -> None:
        ...
        # Reset state
        self._initialized = {tier: False for tier in self._initialized}
    ...
    def get_adapter_type(self, tier: str) -> Optional[str]:
        """
