...
class Router:
    ...
    async def select_tier(self, entry: MemoryEntry) -> str:
        ...
        if selected not in self.config.tiers:
            ...
            logger.warning(
                f"Selected tier '{selected}' not in config, "
                f"using first available: {selected}"
            )
        
        logger.debug(
            f"Selected tier '{selected}' for entry with importance {importance:.2f}"
        )
        return selected
    ...
    async def route_recall(
        ...
        logger.debug(
            f"Recall complete: {len(results)} results, "
            f"{len(promoted_entries)} promotions"
        )
    ...
    def _update_access_metadata(self, entry: MemoryEntry) -> None:
        ...
        logger.debug(
            f"Updated access metadata for entry {entry.id}: "
    ...
    def get_tier_stats(self, tier: Optional[str] = None) -> Dict[str, Any]:
        """
