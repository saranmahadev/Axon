...
class TestCompactionIntegration:
    ...
    async def test_compact_preserves_searchability(
        ...
        auth_results = await system.recall(
            query="JWT token authentication and security",
            k=5,
            tiers=["persistent"]
        )
        ...
        if len(auth_results) > 0:
            ...
            has_summary = any(
                e.type == MemoryEntryType.EMBEDDING_SUMMARY 
                for e in auth_results
            )
            
            print(f"Search results:")
            for i, entry in enumerate(auth_results[:3], 1):
                print(f"{i}. [{entry.type.value}] {entry.text[:80]}...")
    ...
    async def test_compact_dry_run_no_changes(
        self,
        memory_system_with_compaction,
        summarizer
    ):
