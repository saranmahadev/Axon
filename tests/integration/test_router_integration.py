...
@pytest_asyncio.fixture
async def integration_policy_engine(integration_config, integration_registry, integration_scoring_engine):
...
class TestRouterLifecycle:
    ...
    async def test_multi_entry_lifecycle(self, integration_router):
        ...
        # Forget by filter
        filter_obj = Filter(tags=[])  # Match all since we didn't set tags
...
class TestPerformance:
    ...
    async def test_recall_performance(self, integration_router):
        ...
        start = time.time()
        for _ in range(10):
            results = await integration_router.route_recall("performance test", k=5)
