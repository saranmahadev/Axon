...
class TestRouterInit:
    ...
    def test_init_with_policy_engine(
        self,
        mock_config,
        mock_registry,
        mock_policy_engine
...
class TestRouteStore:
    ...
    @pytest.mark.asyncio
    async def test_store_checks_overflow_with_policy(
        self,
        router_with_policy,
        mock_policy_engine
    ):
...
class TestRouteRecall:
    ...
    async def test_recall_specific_tiers(self, router, mock_registry):
        ...
        results = await router.route_recall("test", k=5, tiers=["session"])
    ...
    @pytest.mark.asyncio
    async def test_recall_handles_query_errors_gracefully(
        self,
        router,
        mock_registry
    ):
...
class TestRouteForget:
    ...
    async def test_forget_updates_statistics(self, router, mock_registry):
        ...
        total_forgets = sum(
            router._tier_stats[t]["forgets"]
            for t in router._tier_stats
        )
