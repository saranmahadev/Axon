...
class TestPromotionScore:
    ...
    def test_promotion_threshold(self):
        ...
        assert not details_below["should_promote"], f"Expected entry_below not to promote but got score {score_below}"
        assert details_above["should_promote"], f"Expected entry_above to promote but got score {score_above}"
...
class TestDemotionScore:
    ...
    def test_demotion_score_components(self):
        ...
        entry = create_test_entry(
            importance=0.5,
            last_accessed=datetime.now() - timedelta(days=40)
        )
