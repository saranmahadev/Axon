...
class TestDemotionDecisions:
    ...
    def test_demote_to_nonexistent_tier(self, policy_engine):
        ...
        should_demote, details = policy_engine.should_demote(
            entry,
            "session",
            "archive"  # Doesn't exist in hierarchy
        )
...
class TestCapacityManagement:
    ...
    def test_should_compact_no_threshold(
        self,
        mock_registry,
        scoring_engine
    ):
...
class TestTierPathResolution:
    ...
    def test_get_demotion_path_from_persistent(self, policy_engine):
        ...
        target = policy_engine.get_demotion_path(
            entry,
            "persistent",
            capacity_pressure=0.9
        )
...
class TestEdgeCases:
    ...
    def test_promotion_path_with_missing_tier_in_policies(
        self,
        mock_registry,
        scoring_engine
    ):
