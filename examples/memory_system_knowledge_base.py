...
async def main():
    ...
    async with MemorySystem(config=config) as kb:
        ...
        ml_docs = await kb.recall(
            ...
            k=10,
            filter=Filter(tags=["machine-learning"])
        )
        ...
        # Filter by metadata
        beginner_docs = [d for d in all_docs if getattr(d.metadata, "difficulty", None) == "beginner"]
        advanced_docs = [d for d in all_docs if getattr(d.metadata, "difficulty", None) == "advanced"]
        ...
        optimization_content = await kb.recall(
        ...
        print(f"✓ Automatically tiered by importance:")
        print(f"    - Critical knowledge: persistent tier")
        print(f"    - Standard docs: session tier")
        print(f"    - Temporary notes: ephemeral tier")
        print()
