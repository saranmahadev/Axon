...
async def simulate_tenant_activity(
...
async def main():
    ...
    print(f"   ✅ Created isolated namespaces for 3 tenants:")
    ...
    for tenant_id, info in tenants.items():
        ...
        await simulate_tenant_activity(
            info["adapter"],
            tenant_id,
            num_entries
        )
    ...
    print(f"   Attempting to access Tenant A data from Tenant B namespace...")
    ...
    if result is None:
        print(f"   ✅ Access denied! Tenant B cannot see Tenant A's data")
        print(f"      Entry '{tenant_a_entry_id}' not found in globex namespace")
    else:
        print(f"   ❌ Isolation breach! Cross-tenant access detected")
    ...
    for tenant_id, info in tenants.items():
        ...
        user_entries = await adapter.query(
            vector=None,
            k=100,
            filter=Filter(user_id=f"{tenant_id}_user_0")
        )
        ...
        session_entries = await adapter.query(
            vector=None,
            k=100,
            filter=Filter(session_id=f"{tenant_id}_session_0")
        )
    ...
    print(f"   Clearing cache for Tenant B (globex) only...")
    ...
    print(f"\n✅ Example complete!")
    print(f"\n💡 Key Takeaways:")
    print(f"   • Namespaces provide complete data isolation between tenants")
    print(f"   • Single Redis instance efficiently serves multiple tenants")
    print(f"   • Each tenant can have independent TTL policies")
    print(f"   • Selective cleanup allows per-tenant cache management")
    print(f"   • Queries and operations are automatically scoped to namespace")
    print(f"   • No risk of cross-tenant data leakage")
    print(f"   • Cost-effective multi-tenancy without data mixing")
