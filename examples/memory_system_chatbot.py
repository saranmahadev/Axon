...
class ChatBot:
    ...
    def _calculate_importance(self, message: str) -> float:
        ...
        if any(word in message_lower for word in ["important", "remember", "always", "never", "prefer"]):
            return 0.9
    ...
    async def _generate_response(self, message: str, context: list) -> str:
        ...
        # Questions
            ...
            return f"That's an interesting question about {message[:30]}... Let me help you with that."
...
async def main():
    ...
    async with MemorySystem(config=config) as memory:
        ...
        preferences = await memory.recall(
            "like prefer love",
            k=20,
            min_importance=0.7
        )
        ...
        user1_history = await memory.recall(
            "user",
            k=10,
            filter=Filter(tags=["user1"])
        )
        ...
        print(f"✓ Maintained separate memory for multiple users")
        print(f"✓ Automatically tiered memories by importance")
        print()
