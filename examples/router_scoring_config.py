...
async def main():
    ...
    test_memories = [
        ("Old, rarely accessed", create_memory("test-1", "Old data", 0.4, access_count=1, days_old=30)),
        ("Recent, frequently accessed", create_memory("test-2", "Hot data", 0.4, access_count=20, days_old=1)),
        ("High importance, new", create_memory("test-3", "Important", 0.9, access_count=0, days_old=0)),
        ("Medium, moderate access", create_memory("test-4", "Medium", 0.5, access_count=5, days_old=7)),
    ]
    ...
    default_score, _ = scoring_engine.calculate_promotion_score(
        test_entry, "session"
    )
    recency_score, _ = recency_focused_engine.calculate_promotion_score(
        test_entry, "session"
    )
